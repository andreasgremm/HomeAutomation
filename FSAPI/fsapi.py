...
#        print r.content
    ...
    @property
    def nav_list(self):
        doc = self.call('LIST_GET_NEXT/netRemote.nav.list/-1', dict(
            maxItems=100,
        ))
        if not doc.status == 'FS_OK':
            return list()
    ...
    def set_nav_selectitem(self, value):
        doc = self.call('SET/netRemote.nav.action.selectItem', dict(value=int(value)))
        return doc.status == 'FS_OK'

    nav_selectitem=property(None, set_nav_selectitem)

    def set_nav_action_navigate(self, value):
        doc = self.call('SET/netRemote.nav.action.navigate', dict(value=int(value)))
        return doc.status == 'FS_OK'
