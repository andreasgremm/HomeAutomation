...
import time, os, atexit, urlparse
import json
import argparse
from signal import *
...
lowPowerDetection = {"Waschmaschine":dict(lowpower=4, period=3, emoji=':potable_water:'), "Trockner":dict(lowpower=300, period=2, emoji=':cyclone:'),
		"supercool plug": dict(lowpower = 1260, period = 1, emoji=':signal_strength:')}


def tplink_discover(hs110List):
	hs110IPList=tplinkHS110.TplinkHS110.hs110Discover(broadcastip,'{"system":{"get_sysinfo":{}}}', debug)
	...
	if msg.topic == 'hs110/status':
		...
		for hs110 in hs110List:
			...
			if hs110.name in msg.payload:
				...
				mqttc.publish('hs110/'+hs110.name+'/highpower', hs110.highPower)
			...
			if hs110.connected:
				...
				if error == 0:
					...
					if jmetering["emeter"]["get_realtime"]["err_code"] == 0:
						hs110.currentPower = jmetering["emeter"]["get_realtime"]["power"]
						if debug:
						...
						if hs110.currentPower > 0.5:
							if hs110.currentPower > lowPowerDetection[hs110.name]["lowpower"] and not hs110.systemRunning:
								hs110.startSystem(lowPowerDetection[hs110.name]["lowpower"],lowPowerDetection[hs110.name]["period"])
								rsp=maschinenstatus.chat.post_message('#maschinenstatus', hs110.name + ' ist angelaufen!', username='TPLINK HS110', icon_emoji=lowPowerDetection[hs110.name]["emoji"] )

							lowhighPeakCount, highlowPeakCount, finishDetected = hs110.calculatePeek();
							if finishDetected:
								rsp=maschinenstatus.chat.post_message('#maschinenstatus', hs110.name + ' ist fertig!', username='TPLINK HS110', icon_emoji=lowPowerDetection[hs110.name]["emoji"])
								hs110.stopSystem()
				...
				if dt.datetime.now() >= hs110.disconnectTime + timedelta(minutes=timeout*5):
					hs110.disconnectTime=dt.datetime.now()
					print hs110
