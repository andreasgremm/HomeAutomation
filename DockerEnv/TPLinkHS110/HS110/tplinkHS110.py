...
#		sock_send.bind(('', port_bind))
		...
		while RUN:
			try:
				...
				if debug:
					print "From IP:  ", ip
        			print "Received: ", len(response),':',TplinkHS110.decrypt(response)
				hs110.append(ip)
	...
	def __repr__(self):
		return '<HS110: %s, IP: %s, Connected: %s>' % (self.__name, self.__ip, self.__connected)
	...
	def update(self):
		...
		if error == 0:
			...
			self.__name = unicode(self.__info["system"]["get_sysinfo"]["alias"])
#			print '<Error-Code: %d, alias: %s>' % (self.__info["system"]["get_sysinfo"]["err_code"],
#					self.__info["system"]["get_sysinfo"]["alias"])
	...
	def startSystem(self, lowPowerDetection, period):
		...
		self.__lowPowerCount = 0
#		print "<System %s: LowPowerDetection: %f Period: %d>" % (self.__name, self.__lowPowerDetection, self.__lowPowerDetectionPeriod)
...
#			print "Received: ", TplinkHS110.decrypt(data[4:])
			...
			return (1, "Cound not connect to host " + self.__ip + ":" + str(self.__port))
	...
	def calculatePeek(self):
		...
		if self.__lowPowerCount == self.__lowPowerDetectionPeriod and self.__systemStarted:
			finishDetected = True

#		print "<Lowpower: %d, Power: %d, Highpower %d, .. Lastpower %d>" % (self.__lowPower, self.__currentPower, self.__highPower, self.__lastPower)
#		print "<Lastlowdif: %d, lowdif: %d, Lasthighdif %d, highdif %d>" % (lastlowdifference, lasthighdifference, lowdifference, highdifference)
...
#		print "<S0: %d, S1: %d, LHpeakC: %d, HLpeakC: %d, LowPowerC: %d>" % (s0, s1, self.lowhighPeakCount, self.highlowPeakCount, self.__lowPowerCount)

		return (self.lowhighPeakCount, self.highlowPeakCount, finishDetected)

	def resetPeekValues(self):
#		print "<ResetStart - Lowpower: %d, Power: %d, Highpower %d, .. Lastpower %d>" % (self.__lowPower, self.__currentPower, self.__highPower, self.__lastPower)
		self.lowhighPeak = False
		self.highlowPeak = False
		self.lowhighPeakCount = 0
		self.highlowPeakCount = 0
		self.lastPower = self.currentPower
#		print "<ResetEnd - Lowpower: %d, Power: %d, Highpower %d, .. Lastpower %d>" % (self.__lowPower, self.__currentPower, self.__highPower, self.__lastPower)
...
if __name__ == "__main__":
	hs110_1 = TplinkHS110('192.168.1.105')
	print hs110_1
	del hs110_1

	hs110 = TplinkHS110('192.168.1.105')
	print hs110
