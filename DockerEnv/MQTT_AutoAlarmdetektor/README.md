# MQTT AutoAlarm Detektor
Dieses Programm dient zur Steuerung der Auto-Alarm-Anlage
...
## MQTT AutoAlarm Detektor Container erzeugen und starten
Im Dockerfile werden beim Start des AutoAlarm Detektors verschiedene Parameter benötigt, die im Python-Programm als Aufrufparameter mit Defaults belegt sind.
...
```
docker build --tag=autoalarmdetektor:prod .
```
...
```
docker build --build-arg buildtime_IP_Brocker="<IP.Adresse>" --build-arg buildtime_Client_Name="<Client-Name>" --tag=autoalarmdetektor:prod .
```
...
```
docker run -d  \
  --name=autoalarmdetektor -e "IP_Brocker=<IP.Adresse" -e "Client_Name=<Client-Name>" \
  --mount source=non-git-local-includes,destination=/non-git-local-includes,readonly \
  --restart unless-stopped \
  autoalarmdetektor:prod
```
