...
from Security.Hue import *

lamp_status= { 'on':['1', 'on', 'True', 'yes'], 'off':['0', 'off', 'False', 'no'], 'status':['?', 'status','help'], 'info':['info']}
client_id =''
...
def on_connect(mosq, obj, flags, rc):
	...
	mqttc.subscribe([("hue/control", 2), ("hue/lamp/#", 2), ("hue/scene/#", 2), ("hue/alarm/#", 2)])
...
def on_message(mosq, obj, msg):
	...
	if msg.topic == 'hue/control':
		...
		if 'scene' in json_load:
			...
			try:
				rstatus=HueController.setLampStatus(json_load['lamp'], json.dumps(json_load['data']))
			except Exception as err:
				print (err)
		if 'all' in json_load:
		...
		if msg.payload.decode() in lamp_status['on']:
			...
			try:
				...
				mqttc.publish("hue/lampstatus/"+str(lamp), str(rstatus),0, False)
			except Exception as err:
				print (err)
		elif msg.payload.decode() in lamp_status['info']:
			try:
				rstatus=HueController.getLampInfo(lamp)
				mqttc.publish("hue/lampinfo/"+str(lamp), str(rstatus),0, False)
			except Exception as err:
				print (err)
...
if __name__ == '__main__':
	...
	parser.add_argument('-c', '--client-id', help="Id des Clients", dest="clientID", default="MQTT_Hue")
	...
	parser.add_argument('-hc', '--hue-controller', help="IP Adress des Hue Controllers", dest="hueController", default="192.168.1.127")
	parser.add_argument('-hu', '--hue-user', help="User fuer den Hue Controller", dest="hueUser", default=DefaultHueUser)
